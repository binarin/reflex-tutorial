...
radioCheckbox ::
  ...
  Dynamic t a ->
  m (Event t a)
radioCheckbox dValue dSelected = mdo

  ePostBuild <- getPostBuild

  cb <- checkbox False $
          def & checkboxConfig_setValue .~ eSetValue

  let
    -- eInitialValue :: Event t a
    -- eInitialValue = current dValue <@ ePostBuild

    eInitialState :: Event t Bool
    eInitialState = (current $ (==) <$> dSelected <*> dValue) <@ ePostBuild

    eChange :: Event t Bool
    eChange = cb ^. checkbox_change

    eNowActive :: Event t Bool
    eNowActive = ffilter id eChange

    eOurSelection :: Event t a
    eOurSelection = current dValue <@ eNowActive

    dMatching :: Dynamic t Bool
    dMatching = (==) <$> dValue <*> dSelected

    eSetValue :: Event t Bool
    eSetValue = leftmost [ eInitialState, updated dMatching ]
...
stockWidget dStock dSelected = do
  let domName = dynText (pName . sProduct <$> dStock)
      ...
      domCheckbox = radioCheckbox (pName . sProduct <$> dStock) dSelected
